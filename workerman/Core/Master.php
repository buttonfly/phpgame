...
class Master
{
    ...
    protected static function forkOneWorker($worker_name)
    {
        ...
        elseif($pid === 0)
        {
            ...
            // 查找worker文件
            if($bootstrap = \Man\Core\Lib\Config::get($worker_name.'.bootstrap'))
            {
                $class_name = basename($bootstrap, '.php');
            }
            else
            {
                $class_name = $worker_name;
                $bootstrap = WORKERMAN_ROOT_DIR . "workers/$worker_name.php";
            }
            
            // 如果有语法错误 sleep 5秒 避免狂刷日志
            if(\Man\Core\Lib\Checker::checkSyntaxError($bootstrap, $class_name))
            {
                sleep(5);
            }
